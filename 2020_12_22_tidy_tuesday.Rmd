...
smalldata %>%
  ggplot(aes(date, bm_ex_rate, color = name)) +
  geom_line() +
  expand_limits(y = 0) +
  facet_wrap(vars(name), scales = 'free_y') +
  theme(legend.position = 'none') +
  labs(x = 'year', y = 'Big Mack Exchange Rate', 
...
smalldata2 %>%
  ggplot(aes(date, usd_raw, color = name)) +
  geom_line() +
  facet_wrap(vars(name)) +
  theme(legend.position = 'none') +
  labs(x = 'year', y = '$ raw index', 
       title = 'Over and Under values of the 5 lowest big mack exchange rate countries')
...
bigmac %>%
  ggplot(aes(gdp_dollar, dollar_price, color = name)) +
  geom_point() +
  theme(legend.position = 'none') +
  labs( x = 'GDP per person', y = 'Big Mack price in US $', title = 'Big mack price vs GDP per person')
